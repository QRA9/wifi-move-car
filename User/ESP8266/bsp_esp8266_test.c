...
void Get_car_order(char *cmd){
	...
	while (ptr != NULL) 
	{
		...
		for(int i = 1; i <= 4; i++)
		{
			speed += ((int)order[i] - '0') * n;
			time += ((int)order[i + 4] - '0') * n;
			n = n / 10;
		}
		if(body == 'a')
		{
			car_turn_left(50,time);
		}
		else if(body == 'd')
		{
			car_turn_right(50,time);
		}
		else if(body == 'i')
		{
			car_init();
...
void ESP8266_CheckRecvDataTest(void)
{
  ...
  if(strUSART_Fram_Record.InfBit.FramFinishFlag == 1)
  {
    ...
    strUSART_Fram_Record .InfBit .FramFinishFlag = 0;                            //接收标志置零
		Get_car_order(strUSART_Fram_Record .Data_RX_BUF);
  ...
  {                                                      
    for(i = 0;i < strEsp8266_Fram_Record .InfBit .FramLength; i++)               
    {
     ...
     Get_car_order(strEsp8266_Fram_Record .Data_RX_BUF);
